...
local function create_transition(name)
	...
	local function transition(self, ...)
		if self.asyncState == NONE then
			...
			if not can then
				logerr(string.format("'%s' can't transition from '%s' to '%s'", name, from, to))
				return false
			end
...
function statemachine:can(e)
	...
	if (not to) and (event.map._every) then
		to = event.map._every
	end
	--第二个判断禁止from和to状态相同
	return (to ~= nil) and (to ~= self.current), to
end
